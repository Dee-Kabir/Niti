...
class PendingAppointments extends Component {
    ...
    loadAppointments = async () => {
        ...
        const appointmentRef = await (await doctorsRef.doc(`${this.props.doctorId ? this.props.doctorId : isAuthenticated()}`).collection('appointments').doc("pendingAppointments").get()).data().appointments
        appointmentRef && appointmentRef.map(async(element) => 
         element.get().then((data)=> data.data().doctor.get().then(docData => this.setState((prevAppointmnets)=>({appointments: [...prevAppointmnets.appointments,{doctorName : docData.data().name,created: data.data().created,completed: data.data().completed,appointmentId: data.id}]}))))
        )
        this.setState({loading:false})
    }
    makeAppointmentDone = (id) => {
        this.setState({loading:true})
        const doctorRef = doctorsRef.doc(`${this.props.doctorId ? this.props.doctorId : isAuthenticated()}`)
        const doctorsRefu = doctorsRef.doc(`${this.props.doctorId ? this.props.doctorId : isAuthenticated()}`).collection("appointments")
        ...
        firebase.firestore().runTransaction(async(transaction) =>{
            ...
            if(!docTransaction){
                ...
                transaction.update(doctorsRefu.doc("pendingAppointments"),{appointments : firebase.firestore.FieldValue.arrayRemove(appointmentRef)})
                    .update(doctorsRefu.doc("completedAppointments"),{appointments: firebase.firestore.FieldValue.arrayUnion(appointmentRef)})
        ...
        }).catch(err => this.setState({error: "Please try again",loading:false}))
    }
    render(){
    ...
    return(!loading ? (appointments.length > 0 ?
        ...
        <Table.Body>
        {
            appointments.map((app,_)=>(
                ...
                <Table.Cell >{app.created.toDate().toLocaleString()}</Table.Cell>
                <Table.Cell ><Input onChange={() => this.makeAppointmentDone(app.appointmentId)} type="checkbox" /> </Table.Cell>
                </Table.Row>
            ))
        }
        </Table.Body>
        </Table>
        </div> :<Header>No appointment Yet</Header>) : <LoadingComponent loading={loading} />
    )
    }
}
