    ...
    return(!loading ? (appointments.length > 0 ?
        ...
        <Table.Body>
        {
            appointments.map((app,_)=>(
                ...
                <Table.Cell>{app.completed ? <Link to="#">Raise issue</Link> : <Link to={`/join-room?host=${false}&name=${this.props.user.name}&doctorId=${(app.doctorId).substr(1,12)}`}>Video call</Link>}</Table.Cell>
                </Table.Row>
            ))
        }
        </Table.Body>
